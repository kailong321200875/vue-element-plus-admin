...
| 字段           | 说明                                         |
| -------------- | -------------------------------------------- |
| `visitedViews` | 当前打开的轻量标签，只保存渲染和跳转所需字段 |
| `cachedViews`  | 需要被 KeepAlive 缓存的路由组件名            |

访问路由时，TagsView 会把 Vue Router 路由转换成独立的 `TagView` 数据，再按 `path` 新增或更新标签；查询参数变化会更新已有标签信息，不重复创建同一路径的标签。Store 不保存完整路由对象，避免与 Vue Router 的内部类型和版本耦合。
...
当前不持久化。动态路由会在刷新时重新请求，历史标签对应的权限或路由也可能失效。若业务确实需要恢复标签，建议只保存 `fullPath` 列表，并在动态路由注册完成后逐个校验再恢复。
